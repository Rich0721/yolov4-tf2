...
def bbox_iou(boxes1, boxes2):
    ...
    return 1.0 * intersection_area / (union_area + K.epsilon()), union_area
...
def loss_layer(conv, pred, label, bboxes, stride, num_class, iou_loss_thresh):
    ...
    # loss weight of th gt bbox: 2 - (gt_area/img_area)
    bbox_loss_scale = 2.0 - ((label_xywh[:, :, :, :, 2:3] * label_xywh[:, :, :, :, 3:4] * 1.0) / (input_size * input_size))
    ...
    iou = bbox_iou(expand_pred_xywh, expand_bboxes) #(?, grid_h, grid_w, 3, 70)
    max_iou = tf.expand_dims(tf.math.reduce_max(iou, axis=-1), axis=-1) # (?, grid_h, grid_w, 3, 1)
    ...
    ciou_loss = tf.math.reduce_mean(tf.math.reduce_sum(ciou_loss, axis=[1, 2, 3, 4]))
    conf_loss = tf.math.reduce_mean(tf.math.reduce_sum(conf_loss, axis=[1, 2, 3, 4]))
    prob_loss = tf.math.reduce_mean(tf.math.reduce_sum(prob_loss, axis=[1, 2, 3, 4]))

    return ciou_loss, conf_loss, prob_loss


def decode(conv, anchors, stride, num_class):

    conv_shape = tf.shape(conv)
    batch_size = conv_shape[0]
    output_size = conv_shape[1]
    anchor_per_scale = len(anchors)
    conv = tf.reshape(conv, (batch_size, output_size, output_size, anchor_per_scale, 5+num_class))
    ...
    pred_xy = (tf.sigmoid(xy) + xy_grid) * stride
    pred_wh = (tf.exp(wh) * anchors)
    pred_xywh = tf.concat([pred_xy, pred_wh], axis=-1)
    pred_conf = tf.sigmoid(conf)
    pred_prob = tf.sigmoid(prob)
